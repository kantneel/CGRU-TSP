...
def valid_loss(matrix, rowcol_coef, equal_coef, v):
	...
	basic_loss = -10 * equal_coef * tf.nn.l2_loss(matrix - np.eye(v))

	return rowcol_loss + equal_loss + basic_loss
...
def cycle_length_diff(res, graph, label, vertices):
	...
	retval = tf.cond(out_of_bounds_fail + exactness_fail > 0.05, 
					 lambda:tf.sqrt(tf.square(res_length - label_length)), lambda:res_length - label_length)

	#retval negative -> doing better than label. 
